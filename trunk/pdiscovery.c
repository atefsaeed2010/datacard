...
const struct pdiscovery_device * pdiscovery_lookup_ids(const char * name, int len)
{
	...
	if(pdiscovery_get_id(name, len, "idVendor", &vid) == 1 && pdiscovery_get_id(name, len, "idProduct", &pid) == 1) {
		ast_debug(1, "[datacard discovery] searching for idVendor %04x idProduct %04x\n", vid, pid);
...
//					ast_debug(1, "[datacard discovery] found port %s\n", dentry->d_name);
...
//		ast_debug(1, "[datacard discovery] bInterfaceNumber %02x\n", *interface);
	...
	if(dir) {
		while((dentry = readdir(dir)) != NULL) {
			if(strchr(dentry->d_name, ':')) {
				...
				if(port) {
					ast_debug(1, "[datacard discovery] found InterfaceNumber %02x port %s\n", interface, port);
					for(len2 = 0; len2 < (int)ITEMS_OF(device->interfaces); len2++) {
						if(device->interfaces[len2] == interface) {
							if(ports->ports[len2] == NULL) {
								...
								ast_debug(1, "[datacard discovery] port %s for bInterfaceNumber %02x already exists new is %s\n", ports->ports[len2], interface, port);
								// FIXME
							}
						}
					}
...
#/* */
static char * pdiscovery_handle_ati(char * str, char ** imei2)
{
	...
	if(imei) {
		...
		if(str - imei == IMEI_SIZE) {
			...
			*imei2 = ast_strdup(imei);
			ast_debug(1, "[datacard discovery] found IMEI '%s'\n", imei);
			str[0] = save;
		}
...
#/* */
static void pdiscovery_handle_cimi(char * str, char ** imsi2)
{
	char * imsi;
	...
	if(str - imsi == IMSI_SIZE && str[0] == '\r' && imsi[-1] == '\n') {
		...
		*imsi2 = ast_strdup(imsi);
		ast_debug(1, "[datacard discovery] found IMSI '%s'\n", imsi);
	}
}

#/* return non-zero on done with command */
static int pdiscovery_handle_response(const struct iovec iov[2], int iovcnt, char * info[2])
{
	int done = 0;
	char * str;
	...
	if(len > 0) {
		...
		ast_debug(1, "[datacard discovery] < %s\n", str);
		done = strstr(str, "OK") != NULL || strstr(str, "ERROR") != NULL;
		str = pdiscovery_handle_ati(str, &info[0]);
		pdiscovery_handle_cimi(str, &info[1]);
	}
	return done;
}

#/* return zero on sucess */
static int pdiscovery_do_cmd(int fd, const char * name, const char * cmd, unsigned length, char * info[2])
{
	int timeout;
	char buf[2*1024];
	...
	if(wrote == length) {
		...
		while(timeout > 0 && at_wait(fd, &timeout) != 0) {
			...
			if(iovcnt >= 0) {
				...
				if(pdiscovery_handle_response(iov, iovcnt, info))
					return 0;
			} else {
				break;
			}
...
static int pdiscovery_read_info(struct pdiscovery_ports * ports, char * info[2])
{
	...
	if(cport && strcmp(cport, dport) != 0) {
		...
		if(pid == 0) {
			...
			if(fd >= 0) {
				/* clean queue first ? */
				fail = pdiscovery_do_cmd(fd, cport, cmd, STRLEN(cmd), info);
				closetty(fd, &clock);
			}
			closetty(-1, &dlock);
		} else {
//			ast_debug(1, "[datacard discovery] %s already used by process %d\n", dport, pid);
		...
		if(fd >= 0) {
			/* clean queue first ? */
			fail = pdiscovery_do_cmd(fd, dport, cmd, STRLEN(cmd), info);
			closetty(fd, &dlock);
		}
	}
	return fail;
...
static int pdiscovery_check_imei(struct pdiscovery_ports * ports, struct pdiscovery_req * req)
{
	...
	int match = 0;
	if(pdiscovery_read_info(ports, info) == 0) {
		...
		ast_debug(1, "[datacard discovery] %smatched rIMEI=%s fIMEI=%s rIMSI=%s fIMSI=%s\n",
			match ? "" : "un" , 
		...
		while((dentry = readdir(dir)) != NULL) {
			if(strstr(dentry->d_name, "usb") == NULL && strchr(dentry->d_name, ':') == NULL && strchr(dentry->d_name, '.') == NULL) {
				...
				if(device) {
					...
					ast_debug(1, "[datacard discovery] found device info with %04x %04x modemI=%02x voiceI=%02x dataI=%02x\n", 
						device->vendor_id, 
						device->product_id, 
